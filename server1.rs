...
async fn send_multicast_to_servers(message: u32) -> Result<(), Box<dyn Error>> {
    ...
    let servers= read_ipaddresses("servers.txt".to_string());
    let local_address= servers[0];
    let multicast_to_server_port = 8083;
    ...
    unsafe{
        server_data.local_cpu_usage = cpu;
    }
    let message = format!("{:?} {:?}", message, cpu);
...
async fn encrypt_message(client_addr:Ipv4Addr) -> Result<(), Box<dyn Error>>{
    ...
    let message = format!("{} {}", local_port, "server1");
    println!("Sending message to client {}:{}", client_addr, 8082);
...
{
    let servers= read_ipaddresses("servers.txt".to_string());
    let local_address: Ipv4Addr= servers[0];
    let send_port = 8082;
    ...
    if let Some(request) = pending_requests.get(0).cloned() {
        ...
        if let Some(index) = pending_requests.iter().position(|a_request| a_request.id == request.id) 
        {
            ...
            else if request_type =="online"
            {
                ...
                println!("Removing request at index {}", index);
                pending_requests.remove(0);
                unsafe {
                    server_data.local_load -= 1.0;
                }
...
fn elect_leader() -> Result<(bool), Box<dyn Error>> {
    ...
    // println!("local usage is {:?}", local_usage);
    ...
    loop {
        ...
        if let Some(_ipv4) = extract_ipv4_from_socket_addr(addr) {
            ...
            else if request_type=="unhandled"
            {
                    ...
                    //client_message = client_message.to_string() + " " + &image_id.to_string() + " " + &access_rights.to_string();
                    println!("client unhandled message is {:?}", client_message);
                    //push the request to requests
                    let mut requests = REQUESTS.lock().unwrap();
            ...
            // Acquire the lock in a scoped block
            {
                let mut directory = DIRECTORY_OF_SERVICE.lock().unwrap();
...
async fn receive_failure_from_servers()-> Result<(), Box<dyn Error>>
{
    let servers= read_ipaddresses("servers.txt".to_string());
    let local_address= servers[0];
    ...
    // println!("Listening for messages on {}:{}", local_address, failure_port);
    let (size , addr) = failure_socket.recv_from(&mut buffer).await?;
        ...
        if let Some(ipv4) = extract_ipv4_from_socket_addr(addr) {
            ...
            if title == "Recover" {
                // println!("server {:?} has recovered", ipv4);
                recover_from_failure(server_id.parse::<usize>().unwrap());
                if server_id.parse::<usize>().unwrap() == 1 {
                    // println!("server 2 has recovered and i take the token ");
                    unsafe{
                        server_data.has_failure_token = true;
                    }
                    ...
                    // println!("server 3 has recovered and i dont take the token ");
                    unsafe{
                        server_data.has_failure_token = false;
                    }
            ...
            if title == "take"
            {
                // println!("server 2 did not fail and i take the token");
                if server_id.parse::<usize>().unwrap() == 1 {
                    unsafe{
                        server_data.has_failure_token = true;
                    }
                    ...
                    // println!("server 3 did not fail and i dont take the token");
                    unsafe{
                        server_data.has_failure_token = false;
                    }
...
async fn receive_multicast_from_server2() -> Result<(), Box<dyn Error>> {
    let mut servers = read_ipaddresses("servers.txt".to_string());
    let local_address = servers[0];
    let multicast_from_server_port = 8086;
    ...
    loop {
        ...
        if let Some(ipv4) = extract_ipv4_from_socket_addr(addr) {
            ...
            *counter += 1;
            // println!("Counter in server 2: {}", *counter);
            // Assuming the received_message contains the CPU usage value
            let message_recieved = received_message.to_string();
            ...
            // Update the CPU usage for the corresponding server in the struct
            unsafe{
...
async fn receive_multicast_from_server3() -> Result<(), Box<dyn Error>> {
    let mut servers = read_ipaddresses("servers.txt".to_string());
    let local_address = servers[0];
    let multicast_from_server_port_2 = 8084;
...
async fn check_counter() -> Result<(), Box<dyn Error>> {
    loop{
            ...
            let mut counter = COUNTER_MUTEX.lock().unwrap();
            let found = servers2.iter().any(|&i| i == servers[0]);
...
async fn failure (load:f32) -> Result<(), Box<dyn Error>> {
    ...
    let mut recover = true;
        ...
        loop {
            ...
            // println!("server1 has failure token");
            let now = Instant::now();
...
fn main() -> Result<(), Box<dyn Error>> { 
    ...
    let receive_multicast_from_server_handle = thread::spawn({
        move || {
            if let Err(err) = async_std::task::block_on(receive_multicast_from_server2()) {
                eprintln!("Error in receive_multicast_from_server_handle: {}", err);
            }
        }
    });
    let receive_multicast_from_server_handle_2 = thread::spawn({
        move || {
            if let Err(err) = async_std::task::block_on(receive_multicast_from_server3()) {
                eprintln!("Error in receive_multicast_from_server_handle_2: {}", err);
            }
        }
    });
    let recieve_failure_from_servers_handle = thread::spawn(move || {
        if let Err(err) = async_std::task::block_on(receive_failure_from_servers()) {
            eprintln!("Error in recieve_failure_from_servers_handle: {}", err);
        }
    });
    let check_counter_handle = thread::spawn(move || {
        if let Err(err) = async_std::task::block_on(check_counter()) {
            eprintln!("Error in check_counter_handle: {}", err);
        }
    });

    
    let multicast_handle = thread::spawn(move || {
        // Here, the closure takes ownership of `server_data`
        if let Err(err) = async_std::task::block_on(receive_multicast_messages(message)) {
            eprintln!("Error in multicast_handle : {}", err);
        }
    });
    let failure_handle = thread::spawn(move || {
        
        if let Err(err) = async_std::task::block_on(failure(usage)) {
            eprintln!("Error in failing : {}", err);
        }
    });



    multicast_handle.join().expect("Multicast thread panicked");
    failure_handle.join().expect("failure thread panicked");
    recieve_failure_from_servers_handle.join().expect("Multicast thread panicked");
    receive_multicast_from_server_handle_2.join().expect("Multicast from server 3 thread panicked");
    receive_multicast_from_server_handle.join().expect("Multicast from server 2 thread panicked");
    check_counter_handle.join().expect("check counter thread panicked");
