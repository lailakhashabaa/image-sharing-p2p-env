...
async fn encrypt_message(client_addr:Ipv4Addr) -> Result<(), Box<dyn Error>>{
    ...
    let message = format!("{} {}", local_port, "server3");
...
{
    let servers= read_ipaddresses("servers.txt".to_string());
    let local_address: Ipv4Addr= servers[2];
    let send_port = 8082;
...
async fn send_multicast_to_servers(message:u32) -> Result<(), Box<dyn Error>> {
    ...
    let servers= read_ipaddresses("servers.txt".to_string());
    let local_address= servers[2];
    let multicast_to_server_port = 8086;
...
async fn send_failure_multicast_to_servers(recover : bool,share_token:bool)-> Result<(), Box<dyn Error>>
{
    ...
    for server in &servers {
        if *server != local_address{
        //println!("Sending message to server {}:{}", server, failure_port);
        ...
        if let Some(ipv4) = extract_ipv4_from_socket_addr(addr) {
            ...
            if title == "Recover" {
                ...
                if server_id.parse::<usize>().unwrap() == 0 {
                    unsafe{
                       // println!("server 1 recovered and i am taking the token");
                        server_data.has_failure_token = true;
                    }
            ...
            {
                //println!(" server 1 did not fail and i am taking the token");
                unsafe{
                    server_data.has_failure_token = true;
                }
            }
...
async fn receive_failure_from_server2() -> Result<(), Box<dyn Error>>
{
    let servers= read_ipaddresses("servers.txt".to_string());
    let local_address= servers[2];
    ...
    loop{
        ...
        if let Some(ipv4) = extract_ipv4_from_socket_addr(addr) {
           // println!("Server failed IN FIRST IF: {:?}", ipv4);
            ...
            {
               // println!(" server 2 did not fail and i am not taking the token");
                unsafe{
                    server_data.has_failure_token = true;
                }
            }
            ...
            unsafe{
                if server_data.has_failure_token == true {
                    ...
                    if now.duration_since(last_send_time) >= interval {
                        ...
                        if recover ==true{
                            recover_from_failure(2);
                            //call update directory of service
                     ...
                     if load != 0.0 {
                        //println!("load is not 0 sending token");
...
async fn receive_multicast_from_servers() -> Result<(), Box<dyn Error>> {
    let mut servers = read_ipaddresses("servers.txt".to_string());
    let local_address = servers[2];
    let multicast_from_server_port = 8085;
    let mut servers2 = read_ipaddresses("servers2.txt".to_string());
...
async fn check_counter() -> Result<(), Box<dyn Error>> {
    loop{
        ...
        let mut counter = COUNTER_MUTEX.lock().unwrap();
        let found = servers2.iter().any(|&i| i == servers[2]);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let servers= read_ipaddresses("servers.txt".to_string());
    unsafe{
    server_data.local_address = servers[2];
    server_data.local_id = 3;
    server_data.local_cpu_usage = cpu_usage;
    ...
    let local_id;
    unsafe {
        usage = server_data.local_cpu_usage.clone();
    ...
    let receive_multicast_from_server_handle = thread::spawn({
        move || {
            if let Err(err) = async_std::task::block_on(receive_multicast_from_servers()) {
    ...
    let failure_handle = thread::spawn(move || {
        ...
        if let Err(err) = async_std::task::block_on(receive_failure_from_server1()) {
            eprintln!("Error in receiving failure messages: {}", err);
        }
    });
    let failure_handle_2 = thread::spawn(move || {
        // Here, the closure takes ownership of `server_data`
        if let Err(err) = async_std::task::block_on(receive_failure_from_server2()) {
            eprintln!("Error in receiving failure messages: {}", err);
        }
    });
    let check_counter_handle = thread::spawn(move || {
        ...
        if let Err(err) = async_std::task::block_on(check_counter()) {
            eprintln!("Error in checking counter: {}", err);
        }
    });
    let send_failure_handle = thread::spawn(move || {
        // Here, the closure takes ownership of `server_data`
        if let Err(err) = async_std::task::block_on(failure(usage)) {
            eprintln!("Error in sending failure messages: {}", err);
        }
    });
  
   
    receive_multicast_from_server_handle.join().expect("Multicast from server thread panicked");
    multicast_handle.join().expect("Multicast thread panicked");
    check_counter_handle.join().expect("check counter thread 3 panicked");
    send_failure_handle.join().expect("send failure thread 4 panicked");
    failure_handle.join().expect("Failure thread 1 panicked");
    failure_handle_2.join().expect("Failure thread 2 panicked");
    Ok(())
}